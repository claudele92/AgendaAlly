...
export const StaffDetail = ({ id }: StaffDetailProps) => {
  ...
  const {
    data,
    isLoading: isServiceMasterLoading,
    isError,
  } = useQuery(
    ["times", serviceMasterId],
    () =>
  ...
  return (
    <div>
      ...
      <BookingDateTime
        isError={isError}
        isLoading={isServiceMasterLoading}
        data={data?.data[0].times}
        withBorder={false}
        serviceMasterId={Number(serviceMasterId)}
        master={staffDetail?.data}
      />
    </div>
  );
};
