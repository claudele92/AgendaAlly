...
const CategoryList = ({
  ...
  function goToEdit(uuid) {
    ...
    navigate(`/category/${uuid}`, { state: { parentId, parentUuid } });
  }
  function goToShow(uuid) {
    dispatch(
      addMenu({
        url: `category/show/${uuid}`,
        id: 'category_show',
        name: t('category.show'),
      }),
    );
    navigate(`/category/show/${uuid}`, { state: { parentId, parentUuid } });
  }
  ...
  const [columns, setColumns] = useState([
    ...
    },
    {
      ...
      is_show: true,
      render: (img, row) => {
        return <ColumnImage image={img} id={row?.id} />;
      },
    },
    {
      title: t('active'),
